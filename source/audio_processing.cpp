...
int16_t normalize_audio(std::vector<std::vector<double>>& samples)
{
  ...
  for (const auto& channel : samples) {
    const auto channel_max = std::max_element(
        channel.begin(),
        channel.end(),
...
std::vector<std::vector<double>> frame_slice(const std::vector<double>& samples,
                                             size_t frame_size,
                                             double overlap_ratio)
{
  std::vector<std::vector<double>> frames;
  const auto overlap =
      static_cast<size_t>(static_cast<double>(frame_size) * overlap_ratio);
...
std::vector<double> generate_hamming_window(size_t window_size)
{
  ...
  for (size_t n = 0; n < window_size; n++) {
    hamming[n] = coefficient
        - (1 - coefficient)
            * std::cos((2 * std::numbers::pi * static_cast<double>(n))
...
std::vector<std::vector<double>> spectral_subtraction(
    ...
    const std::vector<double>& noise_profile)
{
  ...
  auto forward_plan =
      fftw_plan_unique_ptr {fftw_plan_dft_r2c_1d(static_cast<int>(frame_size),
                                                 fft_in.get(),
                                                 fft_out.get(),
                                                 FFTW_ESTIMATE)};
  ...
  auto backward_plan =
      fftw_plan_unique_ptr {fftw_plan_dft_c2r_1d(static_cast<int>(frame_size),
                                                 fft_out.get(),
                                                 cleaned_ifft_in.get(),
                                                 FFTW_ESTIMATE)};
  ...
  for (const auto& frame : frames) {
    ...
    for (auto [noise_frame, fft_frame] :
         std::views::zip(noise_profile, fft_out_span))
    {
    ...
    clean_frames.emplace_back(cleaned_ifft_in_span.begin(),
                              cleaned_ifft_in_span.end());
  }

  return clean_frames;
}

std::vector<double> get_noise_profile(
    const std::vector<std::vector<double>>& frames,
...
std::vector<double> overlap_add(const std::vector<std::vector<double>>& frames,
                                size_t frame_size,
                                double overlap_ratio)
{
  const auto overlap =
  ...
  const auto hop = frame_size - overlap;  // This is the size of the frame
                                          // ignoring the overlapped section
  ...
  // NOTE: there is a way to do this more efficiently using the fact that the
  // hamming window is repeated but im lazy (and the beginning and tail ends
  // won't have the same weight pattern)
...
std::vector<int16_t> scale_samples_and_clamp_to_int16(
    const std::vector<double>& normalized_mono_samples, int16_t max)
{
  ...
  double scale = max > 0
      ? static_cast<double>(std::numeric_limits<int16_t>::max()) / max
      : 1.0;
  ...
  for (const auto& sample : normalized_mono_samples) {
    ...
    scaled_sample =
        std::max(scaled_sample,
                 static_cast<int32_t>(std::numeric_limits<int16_t>::min()));
    scaled_sample =
        std::min(scaled_sample,
