...
auto main(int argc, char* argv[]) -> int
{
  ...
  app.add_option("output-file", output_file, "Silenced output file.")
      ->required();

  parallel_audio_processor::options opts {};
  app.add_option("--threads",
                 opts.num_threads,
                 "Number of threads to use while processing audio. Default is "
                 "number of threads in system.")
      ->capture_default_str();

  app.add_option(
         "--noise-frames",
         opts.num_noise_frames,
         "Number of frames to count as noise frames when analyzing audio")
      ->capture_default_str();
